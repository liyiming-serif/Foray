...
///scr_spawn_balloon(wpn_name=rand)

if(argument_count>0){
    var wpn_name = argument[0];
    scr_spawn_balloon_helper(false, wpn_name);
}
else{
    scr_spawn_balloon_helper(false);
}

scr_add_design_pattern("scr_spawn_balloon");

#define scr_spawn_amr_balloon
///scr_spawn_amr_balloon(wpn_name=rand)

if(argument_count>0){
    var wpn_name = argument[0];
    scr_spawn_balloon_helper(true, wpn_name);
}
else{
    scr_spawn_balloon_helper(true);
}

scr_add_design_pattern("scr_spawn_amr_balloon");

#define scr_spawn_balloon_helper
//scr_spawn_balloon_helper(is_armored, wpn_name=rand)
...
scr_balloon_create(pos[0],pos[1],wpn_name,is_amred,undefined,dest_x,dest_y);
